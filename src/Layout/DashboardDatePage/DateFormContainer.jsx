...
export const DateFormContainer = ({
  search,
  setSearch,
  setFilterDate,
  setFilterHour,
  ...
  const SubmitToFilter = (e) => {
    e.preventDefault();
    const data = Form().HandleSubmit(e);
    const { filterByDate, sortByTime } = data;
    setFilterDate(calculateDateRange(filterByDate));
    setFilterHour(sortByTime);
    goToPage(1);
  };
  return (
    <div className="w-full max-w-5xl bg-purple-100 rounded-2xl p-4 flex flex-wrap gap-3 items-center justify-between shadow-sm mt-4">
      ...
      >
        ...
        <div className="flex gap-3">
          ...
          <button className="bg-gray-900 hover:bg-gray-800 text-white text-sm px-4 py-2 rounded-lg shadow-sm cursor-pointer" onClick={()=>navigate("/newDate")}>
            GENERAR CITA
          </button>
        </div>
      </form>
